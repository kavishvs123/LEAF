...
Fine-tunes LLaMA on traffic prediction selection using standard LoRA (full
bfloat16 precision, no quantization). Uses {dataset}_choices.json where
optimal_idx is the ground truth label for which candidate prediction is best.

Standard LoRA loads the model in bfloat16 (~15GB), which gives higher quality
gradients than QLoRA but requires more VRAM. Requires a GPU with at least 20GB
free after accounting for activations and optimizer states.
...
Usage:
    python finetune_llm_lora.py --dataset PEMS08
    python finetune_llm_lora.py --dataset PEMS08 --num_samples 50000 --num_epochs 1
"""
...
# ── Load model in bfloat16 (standard LoRA — no quantization) ─────────────────
...
class CsvLogCallback(TrainerCallback):
    ...
    def on_log(self, args, state: TrainerState, control: TrainerControl, logs=None, **kwargs):
        if logs is None:
            return
        # Only record steps that have a training loss (skip eval-only logs)
        if 'loss' not in logs:
            return
        row = {
...
log_path = os.path.join(args.output_dir, f'{args.dataset.lower()}_lora_training_log.csv')
...
adapter_path = os.path.join(args.output_dir, f'{args.dataset.lower()}_lora_adapter')
